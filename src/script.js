...
function main(){
	...
	let randQues = generateQuestion(quizData);
	isCorrect(randQues);
	
	document.getElementById("question_id").innerText = randQues.question;
    
	document.getElementById("btn1").innerText = randQues.options[0];
	document.getElementById("btn2").innerText = randQues.options[1];
	document.getElementById("btn3").innerText = randQues.options[2];
	document.getElementById("btn4").innerText = randQues.options[3];

}
//why doesnt vsc show suggestion?
//because it dosent know the type of the parameter so it cant suggest for which we use @type 

function isCorrect(obj_ques){
	document.getElementById("div_id").addEventListener("click", (e)=>{
		...
		if(btn_value.innerText === obj_ques.answer){
